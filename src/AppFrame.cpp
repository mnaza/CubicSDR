...
//     demodPanel->SetSizer(demodTray);
    ...
    bookmarkSplitter = new wxSplitterWindow(this, wxID_BM_SPLITTER, wxDefaultPosition, wxDefaultSize, wxSP_3DSASH | wxSP_LIVE_UPDATE );
...
//    mainSplitter->SplitHorizontally( demodPanel, bookmarkSplitter );
    ...
    vbox->Add(bookmarkSplitter, 1, wxEXPAND | wxALL, 0);
...
// // #endif
// //     } else
//       if (!demod && modemPropertiesUpdated.load()) {
//         ModemArgInfoList dummyInfo;
...
void AppFrame::OnDoubleClickSash(wxSplitterEvent& event)
{
    ...
    //     g = 10.0f/37.0f;
    // } else
    if (event.GetId() == wxID_VIS_SPLITTER) {
        w = mainVisSplitter;
        g = 6.0f/25.0f;
    }
